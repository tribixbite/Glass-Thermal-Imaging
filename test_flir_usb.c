...
int main(int argc, char *argv[]) {
    ...
    // Set alternate interface for interface 2 (has EP 0x85)
    printf("\nSetting alternate interfaces...\n");
    struct usbdevfs_setinterface setif;
    setif.interface = 2;
    setif.altsetting = 0;  // Use Alt 0 which has EP 0x85
    ...
    printf("Init step 1 (interface 2): %d\n", ret);

    // Step 2: Control transfer to interface 1
    ctrl.wIndex = 1;
    ret = ioctl(fd, USBDEVFS_CONTROL, &ctrl);
    printf("Init step 2 (interface 1): %d\n", ret);

    // Step 3: Control transfer to interface 1 with value 1
    ctrl.wValue = 1;
    ret = ioctl(fd, USBDEVFS_CONTROL, &ctrl);
    printf("Init step 3 (interface 1, val 1): %d\n", ret);

    // Send CameraFiles.zip request (from ROS driver)
    printf("\nSending CameraFiles.zip request...\n");
    ...
    // Skip video start since it causes device disconnect
    printf("\nSkipping video start (causes disconnect on Glass)\n");

    // Try reading EP 0x83 for file data
    printf("\nReading EP 0x83 for CameraFiles.zip data...\n");
    bulk.ep = 0x83;
    bulk.len = sizeof(buffer);
    bulk.timeout = 1000;
    bulk.data = buffer;
    ...
    for (int i = 0; i < 5; i++) {
        ret = ioctl(fd, USBDEVFS_BULK, &bulk);
        if (ret > 0) {
            printf("EP 0x83 read %d: Got %d bytes\n", i+1, ret);
    ...
    for (int attempt = 0; attempt < 10 && frames_captured < 3; attempt++) {
