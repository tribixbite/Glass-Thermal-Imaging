...
#define EP_VIDEO_READ_SIZE 4096 // A safer, smaller bulk transfer size
...
int main(int argc, char *argv[]) {
    ...
    // CRITICAL: Set video interface to alternate setting 1
    printf("Attempting to switch to high bandwidth mode (Alt 1)...\n");
    setif.interface = 2;
    setif.altsetting = 1;
    ret = ioctl(fd, USBDEVFS_SETINTERFACE, &setif);
    if (ret < 0) {
